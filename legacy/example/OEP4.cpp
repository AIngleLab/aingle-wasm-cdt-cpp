...
class oep4 : public contract {
    ...
    bool init(){
        aingle_assert(check_witness(OWNER),"checkwitness failed");
        asset totalsupply = 0;
    ...
    bool transfer(address from, address to , asset amount){
        aingle_assert(check_witness(from),"checkwitness failed");
        aingle_assert(amount > 0,"amount should greater than 0");
        if (from == to){
            return true;
        }
        ...
        aingle_assert(frombalance >= amount,"amount error");
    ...
    bool approve(address from, address to , asset amount){
        aingle_assert(check_witness(from),"checkwitness failed");
        aingle_assert(amount > 0,"amount error");
        aingle_assert(balanceOf(from) >= amount,"amount error");
    ...
    bool transferFrom(address owner, address from,address to, asset amount){
        ...
        aingle_assert(aw >= amount,"amount error");
        asset frombalance = balanceOf(from);
        aingle_assert(frombalance >= amount,"amount error");
    ...
    bool transferMulti(mul_trans_args params){
        for (struct transferM p :params){
            aingle_assert(transfer(p.from, p.to, p.amount),"tranfer failed");
        }
        return true;
    }
