...
class oep5 : public contract {
    ...
    bool transfer(address to , uint64_t tokenid){
        ...
        aingle_assert(check_witness(oldowner),"checkwitness failed!");
    ...
    bool transferMulti(std::vector<struct transferM> params){
        for (struct transferM p :params){
            aingle_assert(transfer(p.to, p.tokenID),"tranfer failed");
        }
        return true;
    }
    ...
    bool takeOwnership(address to,uint64_t tokenID){
        ...
        aingle_assert(check_witness(t),"checkwitness failed!");
    ...
    bool createToken(std::string name,std::string image,std::string description,std::string other){
        aingle_assert(check_witness(OWNER),"check witness failed");
...
AINGLE_DISPATCH(oep5, (init)(transfer)(balanceOf)(ownerOf)(takeOwnership)(approve)(name)(symbol)(totalSupply)(createToken)(getTokenByID));
