...
#include <iostream>
#include <typeinfo>
#include <aingle/gen.hpp>
#include <aingle/whereami.hpp>
#include <aingle/abi.hpp>
...
struct abigen_exception : public std::exception {
   virtual const char* what() const throw() {
      return "aingle.abigen fatal error";
   }
} abigen_ex;

DeclarationMatcher function_decl_matcher = cxxMethodDecl().bind("aingle_abis");
StatementMatcher stringLiteral_matcher  = stringLiteral(hasParent(implicitCastExpr(hasParent(callExpr(hasParent(constantExpr()), hasDescendant(declRefExpr(to(functionDecl(hasName("hash_at_compile_time"))))))))), hasAncestor(caseStmt(has(constantExpr(has(callExpr(hasDescendant(declRefExpr(to(functionDecl(hasName("hash_at_compile_time"))))))))), hasDescendant(stringLiteral()), hasAncestor(functionDecl(allOf(hasName("invoke"), hasParent(linkageSpecDecl()),isExternC(),parameterCountIs(0), returns(asString("void")))))))).bind("aingle_abis");
...
class abigen : public generation_utils {
   ...
   std::string generate_json_comment() {
      std::stringstream ss;
      ss << "This file was generated with aingle-abigen.";
      ss << " DO NOT EDIT";
      return ss.str();
   }
...
class aingleMethodMatcher : public MatchFinder::MatchCallback {
   public:
      virtual void run( const MatchFinder::MatchResult& res ) {
         if (const clang::CXXMethodDecl* decl = res.Nodes.getNodeAs<clang::CXXMethodDecl>("aingle_abis")->getCanonicalDecl()) {
            if (abigen::is_aingle_contract(decl, get_abigen_ref().get_contract_name())) {
...
class aingleAbiMatcher : public MatchFinder::MatchCallback {
   public:
      virtual void run( const MatchFinder::MatchResult& res ) {
         if (const clang::StringLiteral* decl = res.Nodes.getNodeAs<clang::StringLiteral>("aingle_abis")) {
...
int main(int argc, const char **argv) {
   ...
   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
        os << "aingle-abigen version " << "v1.0" << "\n";
  });
   cl::OptionCategory cat("aingle-abigen", "generates an abi from C++ project input");
   ...
   options.push_back(std::string("-I")+aingle::cdt::whereami::where()+"/../include/libcxx");
   options.push_back(std::string("-I")+aingle::cdt::whereami::where()+"/../include/libc");
   options.push_back(std::string("-I")+aingle::cdt::whereami::where()+"/../include/boost");
   options.push_back(std::string("-I")+aingle::cdt::whereami::where()+"/../include/ainglelib");
   options.push_back(std::string("-I")+aingle::cdt::whereami::where()+"/../include");
   ...
   aingleAbiMatcher aingle_abi_matcher;
   MatchFinder finder_stringLiteral;

   aingleMethodMatcher aingle_method_matcher;
   MatchFinder finder_method_decl;

   finder_stringLiteral.addMatcher(stringLiteral_matcher, &aingle_abi_matcher);
   finder_method_decl.addMatcher(function_decl_matcher, &aingle_method_matcher);
