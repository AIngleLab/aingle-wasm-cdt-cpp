...
class transferinner {
    ...
    public:
		...
		static asset balanceof_inner(address addr, address from) {
			...
			aingle::check( balance <= asset::max_amount, "balanceof_inner. balance overflow");
			aingle::check( balance >= -asset::max_amount, "balanceof_inner. balance underflow");
...
class ont: aingle_internal_namespace::transferinner {
	private:
...
class ong: public aingle_internal_namespace::transferinner {
	private:
