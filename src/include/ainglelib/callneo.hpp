...
void notify_event(Args&&... args) {
  uint8_t ltype = aingle_internal_namespace::ListType;
  uint32_t lsize = sizeof...(args);
  size_t size = aingle_internal_namespace::notify_size(args...);
  size = size + aingle_internal_namespace::pack_size(uint32_t(0),ltype, lsize); // for magic "evt\x00" and one byte ArgTListType
  ...
  aingle_internal_namespace::notify_event_buff(ds, args...);
  notify(result);
}
...
std::vector<char> pack_neoargs(Args&&... args) {
  ...
  uint32_t lsize = sizeof...(args);
  size_t size = aingle_internal_namespace::notify_size(args...);
  size = size + aingle_internal_namespace::pack_size(magic_version,ltype, lsize); // for magic "evt\x00" and one byte ArgTListType
  std::vector<char> result;
  result.resize(size);
  datastream<char*> ds( result.data(), result.size());
