   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        aingle::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        aingle::check( _end - _pos >= (int32_t)s, "write" );
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      aingle::check(false, "invalid variant index");
   }
}
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   aingle::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
...
std::vector<char> pack( const T& value ) {
  ...
  result.resize(aingle_internal_namespace::pack_size(value));
...
std::vector<char> pack(Args&&... args) {
  std::vector<char> result;
  result.resize(aingle_internal_namespace::pack_size(args...));
  datastream<char*> ds( result.data(), result.size());
