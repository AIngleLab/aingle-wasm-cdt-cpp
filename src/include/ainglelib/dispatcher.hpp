...
namespace aingle {
    ...
    *
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from aingle::contract
 ...
 // Helper macro for AINGLE_DISPATCH_INTERNAL
 #define AINGLE_DISPATCH_INTERNAL( r, OP, elem ) \
	case __hash_string__::hash_at_compile_time( BOOST_PP_STRINGIZE(elem) ) : \
       aingle::execute_action( ds, &OP::elem ); \
       break;

 // Helper macro for AINGLE_DISPATCH
 #define AINGLE_DISPATCH_HELPER( TYPE,  MEMBERS ) \
    BOOST_PP_SEQ_FOR_EACH( AINGLE_DISPATCH_INTERNAL, TYPE, MEMBERS )
 ...
 *
 * @note To be able to use this macro, the contract needs to be derived from aingle::contract
 * @param TYPE - The class name of the contract
 * @param MEMBERS - The sequence of available actions supported by this contract
 *
 * Example:
 * @code
 * AINGLE_DISPATCH( aingle::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#if defined(WASM_LOCAL_DEBUG )
#define AINGLE_DISPATCH( TYPE, MEMBERS ) \
extern "C" { \
   void apply(void) {  \
      std::string method; \
      ...
      switch(__hash_string__::string_hash(method) ) { \
          AINGLE_DISPATCH_HELPER( TYPE, MEMBERS ) \
		  default: \
