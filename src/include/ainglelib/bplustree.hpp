...
bool get_block_bykey(const block_key_t &key, T &value) {
	if (key == invalid_rootblock_key)
		return false;
	return aingle::storage_get(aingle::make_key(block_key_t_prefix,key), value);
}
...
void put_block_bykey(const block_key_t &key, const T &value) {
	if (key == invalid_rootblock_key)
		return;
	aingle::storage_put(aingle::make_key(block_key_t_prefix, key), value);
}
...
private:
    ...
    std::pair<iterator, bool> insert_descend(
        ...
        else
        {
			...
			aingle::check(slot >= 0 && slot <= leaf->slotuse, "slot must be in range.");
